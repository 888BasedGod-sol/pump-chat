...
export default function AppHeader() {
  ...
  useEffect(() => {
    const searchChanged = localSearch !== prevSearchRef.current;
    prevSearchRef.current = localSearch;
    debounceRef.current = setTimeout(() => {
      setSearchQuery(localSearch);
      // Navigate to communities page only when the user actually types (not on pathname change)
      if (searchChanged && localSearch.trim() && pathname !== "/app") {
        router.push("/app");
      }
    }, 150);
    return () => clearTimeout(debounceRef.current);
  }, [localSearch, setSearchQuery, pathname, router]);
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-border bg-surface/75 backdrop-blur-xl relative shadow-[0_6px_20px_rgba(5,10,20,0.35)]">
      ...
      <div className="flex h-12 items-center justify-between px-4">
        <div className="flex items-center gap-4">
          ...
          <Link href="/" className="flex items-center gap-2 text-base font-extrabold text-accent tracking-[0.08em] uppercase">
            SAFEMOON
          </Link>
          <a
            href="https://x.com/Safemoonrrr"
            target="_blank"
          ...
          <div className="hidden md:block">
            <input
              ...
              className="h-8 w-64 rounded-md border border-border bg-background/80 px-3 text-xs text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none"
            />
          </div>
        </div>
        <nav className="hidden items-center gap-4 text-xs font-medium text-text-secondary md:flex">
          ...
          >
            ...
            <span className="pointer-events-none absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-background border border-border px-2 py-1 text-[10px] text-text-muted opacity-0 transition-opacity group-hover:opacity-100 shadow-lg z-50">
              Automated SAFEMOON raid engine - coming soon
            </span>
          </button>
        </nav>
        ...
        <div className="border-t border-border bg-surface p-3 space-y-2 md:hidden">
          ...
          <div className="flex flex-col gap-1 text-xs font-medium text-text-secondary">
            ...
            <div className="flex items-center gap-2 rounded-md border border-accent/20 bg-accent/5 px-2 py-1.5">
              ...
              <span className="text-accent font-bold">Raid Engine</span>
